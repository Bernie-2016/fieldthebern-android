  ...
  static public class Presenter extends ViewPresenter<MapScreenView> {
    ...
    private void promptToContinue() {
      ...
      DialogService.get(getView())
          .setDialogConfig(new DialogController.DialogConfig().title(inProgressTitle)
    ...
    MapScreenView.OnCameraChange onCameraChange = new MapScreenView.OnCameraChange() {
      ...
      public void onCameraChange(CameraPosition cameraPosition, boolean shouldRefreshAddresses,
        ...
        multiAddressSubscription = addressRepo.getMultiple(new AddressSpec().multipleAddresses(
            new RequestMultipleAddresses().latitude(cameraPosition.target.latitude)
    ...
    MapScreenView.OnAddressChange onAddressChange = new MapScreenView.OnAddressChange() {
      ...
      public void onAddressChange(ApiAddress address) {
        ...
        singleAddressSubscription = addressRepo.getSingle(
            new AddressSpec().singleAddress(new RequestSingleAddress(address)))
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
